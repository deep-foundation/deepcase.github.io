...
Particles.prototype = {
  ...
  disintegrate: function (options, onComplete) {
    if (!this.isAnimating()) {
  ...
  integrate: function (options, onComplete) {
    if (!this.isAnimating()) {
  ...
  animate: function (update, onComplete) {
    var _ = this;
    anime({
...
export function ParticlesComponent({ pRef, eRef, options, children }) {
  useEffect(() => {
    pRef.current = new Particles(eRef.current, options);
  }, []);
  return children;
}
